...
train_file = "movielens1m.NeuPR.trn.98129n.implicit.csv"
eval_file  = "movielens1m.NeuPR.val.6040n.implicit.csv"
pred_file  = "movielens1m.NeuPR.prd.355700n.implicit.csv"

def _train_generator(params):
    """Yielding samples one by one of MovieLens 1M dataset for training, with
    the following info:

        ((UserID, MovieID1, MovieID2), Label).
...
def _eval_generator(params):
    ...
    for u in users:
        ...
        user_neg_data = user_data[user_data["Label"] == 0]
        for i in user_pos_data.index:
            for j in user_neg_data.index:
                if user_pos_data.loc[i, "MovieID"] != \
                   user_neg_data.loc[j, "MovieID"]:
                    # positive eval data
                    yield ((user_neg_data.loc[j, "UserID"],
                            user_pos_data.loc[i, "MovieID"],
                            user_neg_data.loc[j, "MovieID"]), 1)
                    # negative eval data
                    yield ((user_neg_data.loc[j, "UserID"],
                            user_neg_data.loc[j, "MovieID"],
                            user_pos_data.loc[i, "MovieID"]), 0)
...
def _pred_generator(params):
    ...
    logging.info("yielding pred samples.")
    users = list(set(data["UserID"].values.tolist()))
    for u in users:
        user_data = data[data["UserID"] == u]
        user_pos_data = user_data[user_data["Label"] == 1]
