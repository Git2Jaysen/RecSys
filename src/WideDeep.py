...
def model_fn(features, labels, mode, params):
    ...
    # shape: [batch_size, embedding_size]
    Wide_output = user_emb_inp * item_emb_inp
    logging.info("Wide_output shape: {}".format(Wide_output.shape))

    logging.info("defining Deep componet.")
    # shape: [batch_size, 7 * embedding_size]
    Deep_input = tf.reshape(
        tf.concat([user_emb_inp,
                   item_emb_inp,
                   gender_emb_inp,
                   age_emb_inp,
                   occupation_emb_inp,
    ...
    logging.info("Deep_output shape: {}".format(Deep_output.shape))

    logging.info("defining Wide&Deep model.")
    # shape: [batch_size, embedding_size + num_factors]
    Wide_Deep_concat = tf.reshape(
        tf.concat([Wide_output, Deep_output], axis = -1),
        shape = [-1, params["embedding_size"] + params["num_factors"]])
    logging.info("Wide_Deep_output shape: {}".format(Wide_Deep_concat.shape))
